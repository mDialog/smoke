...
By default, Smoke relies on [Netty](http://netty.io) to for asynchronous HTTP communication. Alternately, you can use [Mongrel2](http://mongrel2.org):
...
Smoke also provides a simple collection of Request extractors based on the familiar set found in projects like [Play2 Mini](https://github.com/typesafehub/play2-mini) and [Unfiltered](http://unfiltered.databinder.net/Unfiltered.html).

    case GET(Path(Seg("resources" :: id :: Nil))) => ...
    case POST(Path("/resources")) & Params(p) => ...

Use the `reply` method, supplying a function that returns a Response and it will be wrapped in a Future for you.
...
If your request can be responded to quickly and immediately, you can you use `reply` method to wrap your Response in a completed Future.

    case _ => reply(Response(NotFound))

Since a reply is just a Future[Response], you can also ask an actor for your Response.
...
Even better, you can use the tools provided by Akka to compose your function using many Futures:
		...
		object ChainedActorExampleApp extends App with Smoke {
...
You can use the `before` and `after` filters to alter either the request prior to sending it to the responder or the response after it's returned.
